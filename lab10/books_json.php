...
// write a code to :
// 1. read the "books.txt" / OK
// 2. search all the books that matches the given category / OK
...
for ($i = 0; $i < count($lines); $i++) {
	...
	if ($book_category == $category) {
		...
		$obj = array("category" => $book_category, "title" => $title, "author" => $author, "year" => $year, "price" => $price);
		array_push($books, json_encode($obj));
	}
}
print implode(",\n", $books);
